...
def create_metrics_table(metrics: SystemMetrics, summary: dict[str, float], recent_metrics: list[SystemMetrics], alerts: AlertStatus) -> Group:
    ...
    if metrics.cpu_temperature is None:
        ...
        temperature_status = "[dim]Unavailable[/dim]"
    else:
        temp_status, temp_color = get_temperature_status(metrics.cpu_temperature)

        temperature_value = f"{metrics.cpu_temperature:.1f}°C"
        temperature_status = f"[{temp_color}]{temp_status}[/{temp_color}]"
    ...
    summary_table.add_row("CPU Temp", f"{summary['avg_cpu_temperature']:.1f}°C")
    ...
    if alerts.temperature:
        alerts_table.add_row("[bold red]CPU temperature has remained critically high[/bold red]")
    if alerts.ram:
